 ...
 */
public class WeaponBuyScene extends SimpleScene {
//    enum MSG_ID {MSG_SELECT_WEAPON_UP, MSG_SELECT_WEAPON_DOWN, MSG_UP_LEFT, MSG_UP_RIGHT, MSG_DOWN_LEFT, MSG_DOWN_RIGHT}
    ...
    public WeaponBuyScene(boolean isInit) {
        super(SCENE_ID.WEAPON_BUY, 800.0f, 600.0f, GameSetup.deviceWidthPixels, GameSetup.deviceHeightPixels);
      ...
      /*  WeaponSelectFilter filterUp = new WeaponSelectFilter();
            ...
            if (countryFilterSetUp.contains(weaponUp.info.country)) {
                if ((sceneValues.getInt("pageUp") - 1) * 27 <= countUp && countUp < sceneValues.getInt("pageUp") * 27) {
                    ...
                    weaponSprite = new TeamBuildWeaponSprite(x, y,
                            weaponUp, new TouchMessage(MSG_ID.MSG_SCENE_WEAPON_BUY__SELECT_WEAPON_UP, null, this, weaponUp.info.id));
        ...
        if (sceneValues.getInt("pageUp") > 1) {
            NormalSprite liftSprite = new NormalSprite(34, 120, "scroolbt", 0, new TouchMessage(MSG_ID.MSG_SCENE_WEAPON_BUY__UP_LEFT, null, this));
            addSprite(liftSprite);
        }
        if (sceneValues.getInt("pageUp") < (countUp - 1) / 30 + 1) {
            NormalSprite rightSprite = new NormalSprite(703, 120, "scroolbt", 1, new TouchMessage(MSG_ID.MSG_SCENE_WEAPON_BUY__UP_RIGHT, null, this));
        ...
        for (BaseWeapon weapNodeDown : weapNodesDown) {
            ...
            if ((sceneValues.getInt("pageDown") - 1) * 36 <= countDown && countDown < sceneValues.getInt("pageDown") * 36) {
                ...
                weaponSprite = new TeamBuildWeaponSprite(offsetDownX
                        ...
                        weapNodeDown, new TouchMessage(MSG_ID.MSG_SCENE_WEAPON_BUY__SELECT_WEAPON_DOWN, null, this, weapNodeDown.id));
                addSprite(weaponSprite);
        ...
        if (sceneValues.getInt("pageDown") > 1) {
            ...
            NormalSprite liftSprite = new NormalSprite(100, 410, "scroolbt", 0, new TouchMessage(MSG_ID.MSG_SCENE_WEAPON_BUY__DOWN_LEFT, null, this));
            addSprite(liftSprite);
        }
        if (sceneValues.getInt("pageDown") < (countDown - 1) / 36 + 1) {
            NormalSprite rightSprite = new NormalSprite(750, 410, "scroolbt", 1, new TouchMessage(MSG_ID.MSG_SCENE_WEAPON_BUY__DOWN_RIGHT, null, this));
            addSprite(rightSprite);
        }

        NormalSprite back2Sprite = new NormalSprite(0.0f, 0.0f, "oper_bg2", 0, new TouchMessage(MSG_ID.MSG_SCENE_WEAPON_BUY__BACKUP_TOUCH, null, this), 1, 1, 0.0f);
    ...
    public void onHandlMessage(TouchMessage pTouchMessage) {
        ...
        if (msgID == MSG_ID.MSG_SCENE_WEAPON_BUY__UP_LEFT) {
            playSound("scrl01");
        ...
        if (msgID == MSG_ID.MSG_SCENE_WEAPON_BUY__UP_RIGHT) {
            playSound("scrl01");
            sceneValues.setInt("pageUp", sceneValues.getInt("pageUp") + 1);
            WeaponBuyScene weaponBuyScene = new WeaponBuyScene(false);
            SceneManager.render(weaponBuyScene);
        }
        if (msgID == MSG_ID.MSG_SCENE_WEAPON_BUY__DOWN_LEFT) {
            playSound("scrl01");
            sceneValues.setInt("pageDown", sceneValues.getInt("pageDown") - 1);
            WeaponBuyScene weaponBuyScene = new WeaponBuyScene(false);
            SceneManager.render(weaponBuyScene);
        }
        if (msgID == MSG_ID.MSG_SCENE_WEAPON_BUY__DOWN_RIGHT) {
            playSound("scrl01");
            sceneValues.setInt("pageDown", sceneValues.getInt("pageDown") + 1);
            WeaponBuyScene weaponBuyScene = new WeaponBuyScene(false);
            SceneManager.render(weaponBuyScene);
        }
        if (msgID == MSG_ID.MSG_SCENE_WEAPON_BUY__SELECT_WEAPON_UP) {
        ...
        if (msgID == MSG_ID.MSG_SCENE_WEAPON_BUY__SELECT_WEAPON_DOWN) {
        ...
        if (msgID == MSG_ID.MSG_SCENE_WEAPON_BUY__BACKUP_TOUCH) {
            sceneValues.remove("selectedWeaponIDUp");
            sceneValues.remove("selectedWeaponIDDown");
            WeaponBuyScene weaponBuyScene = new WeaponBuyScene(false);
            SceneManager.render(weaponBuyScene);
