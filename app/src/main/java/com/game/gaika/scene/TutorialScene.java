...
public class TutorialScene extends SimpleScene{
...
//    enum MSG_ID {MSG_DELAY_TIME_OUT, MSG_BACKUP_TOUCHE}
    ...
    public TutorialScene(InitID pInitID) {
        super(SCENE_ID.TUTORIAL,  800.0f, 600.0f, GameSetup.deviceWidthPixels, GameSetup.deviceHeightPixels);
        ...
        NormalSprite backSprite = new NormalSprite(0.0f, 0.0f, imageList.get(0).imageName, 0, new TouchMessage(MSG_ID.MSG_SCENE_TUTORIAL__BACKUP_TOUCHE, null, this));
    ...
    public void onHandlMessage(TouchMessage pTouchMessage) {
        ...
        if (EMsgID == MSG_ID.MSG_SCENE_TUTORIAL__BACKUP_TOUCHE) {
             playSound("scrl01");
            if(imageList.size() > 0) {
                imageList.remove(0);
            }
            if (imageList.size() > 0) {
                TutorialScene tutorialScene = new TutorialScene(InitID.INIT_NEXT_PAGE);
