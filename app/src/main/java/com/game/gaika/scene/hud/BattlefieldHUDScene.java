...
//    private  int infoCardWeaponID  = -1;
    ...
    public BattlefieldHUDScene(float pWidth, float pHeight, IMessageHandler pMessageHandler, int pInfoCardWeaponID) {
        super(SCENE_ID.BATTLEFIELD_HUD, pWidth, pHeight);
        ...
        if (pMessageHandler != null) {
            NormalSprite turnFinishButtonSprite = new NormalSprite(width - 158.0f * unZoom, 0.0f * unZoom, "sys_bt2", 1, new TouchMessage(MSG_ID.MSG_SCENE_HUD__TURN_FINISH, null, pMessageHandler));
            turnFinishButtonSprite.setScale(unZoom);
            addSprite(turnFinishButtonSprite);

            NormalSprite systemButtonSprite = new NormalSprite(width - 87.0f * unZoom, 0.0f * unZoom, "sys_bt", 1, new TouchMessage(MSG_ID.MSG_SCENE_HUD__BUTTON_SYSTEM, null, pMessageHandler));
