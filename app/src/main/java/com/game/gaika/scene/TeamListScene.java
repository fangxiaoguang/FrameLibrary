 ...
 */
public class TeamListScene extends SimpleScene{

    //enum MSG_ID {MSG_RETURN, MSG_SCROOL_UP, MSG_SCROOL_DOWN}
    public TeamListScene(boolean pInit, SCENE_ID pParentSceneID) {
        super(SCENE_ID.TEAM_LIST, 800.0f, 600.0f, GameSetup.deviceWidthPixels, GameSetup.deviceHeightPixels);
        ...
        sortsSubSceneUp.setOffset(new Pair<Float, Float>(210.0f, 30.0f));
        addChildScene(sortsSubSceneUp);

        FlagsSubScene flagsSubSceneUp = new FlagsSubScene("Up", this);
        flagsSubSceneUp.setOffset(new Pair<Float, Float>(524.0f, 32.0f));
        addChildScene(flagsSubSceneUp);


        if (sceneValues.getInt("pageUp") > 1) {

            NormalSprite scroolUpSprite = new NormalSprite(298, 60, "scroolb2", 0, new TouchMessage(MSG_ID.MSG_SCENE_TEAM_LIST__SCROOL_UP, null, this));
...
//        }
        ...
        for (BaseWeapon weap : weapNodesUp) {
            ...
            if ((page - 1) * 20 <= countUp && countUp < page * 20) {
                ...
                TextSprite nameSprite = new TextSprite(50, 113 + 21 * (countUp % 20), true, weap.info.name, getFont12());
                addSprite(nameSprite);

                TextSprite typeNameSprite = new TextSprite(179, 113 + 21 * (countUp % 20), true, weap.info.type.toString(), getFont12());
...
//-----------------------------------------------------
        if (sceneValues.getInt("pageUp") < (countUp + 19) / 20) {
            ...
            NormalSprite scroolUpSprite = new NormalSprite(298, 545, "scroolb2", 1, new TouchMessage(MSG_ID.MSG_SCENE_TEAM_LIST__SCROOL_DOWN, null, this));
            addSprite(scroolUpSprite);
        }
        NormalSprite buttonRetSprite = new NormalSprite(35, 556, "retn_bt1", 0, new TouchMessage(MSG_ID.MSG_SCENE_TEAM_LIST__RETURN, null, this));
