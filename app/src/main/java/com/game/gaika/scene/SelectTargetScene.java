 ...
 */
public class SelectTargetScene extends SimpleScene {

    // enum MSG_ID {MSG_SELECT_CHAPTER}
    public SelectTargetScene(boolean isInit) {
        super(SCENE_ID.SELECT_TARGET, 800.0f, 600.0f, GameSetup.deviceWidthPixels, GameSetup.deviceHeightPixels);
...
//            sceneValues.setEnum("sortTypeUp", TYPE);
            ...
            for (GameChapter chapter : gdm.chapters.values()) {
                if (chapter.area == ALL) {
                    if (chapter.finished == true) {
                        ...
                        SelectChapterSprite chapterSprite = new SelectChapterSprite(chapter, isSelected, new TouchMessage(MSG_ID.MSG_SCENE_SELECT_TARGET__SELECT_CHAPTER, null, this, chapter.no));
                        addSprite(chapterSprite);
                    }
        ...
        TextSprite textSprite = new TextSprite(760, 551, false, gdm.getCurrentChapter().cName, getFont16());
    ...
    public void onHandlMessage(TouchMessage pTouchMessage) {
        ...
        SceneValueMap sceneValues = getSceneValuesMap();
        if (msgID == MSG_ID.MSG_SCENE_SELECT_TARGET__SELECT_CHAPTER) {
            playSound("select01");
