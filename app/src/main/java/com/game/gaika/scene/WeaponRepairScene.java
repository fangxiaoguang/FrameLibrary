 ...
 */
public class WeaponRepairScene extends SimpleScene {
...
//    enum MSG_ID {MSG_SELECT_WEAPON_UP, MSG_UP_LEFT, MSG_UP_RIGHT, MSG_REPAIR_ALL, MSG_REPAIR_ALL_DLG_YES, MSG_REPAIR_ALL_DLG_NO}
    ...
    public WeaponRepairScene(boolean isInit) {
        super(SCENE_ID.WEAPON_REPAIR, 800.0f, 600.0f, GameSetup.deviceWidthPixels, GameSetup.deviceHeightPixels);
        ...
        for (BaseWeapon weapNodeUp : weapNodesUp) {
            ...
            if ((sceneValues.getInt("pageUp") - 1) * 45 <= countUp && countUp < sceneValues.getInt("pageUp") * 45) {
                ...
                TeamBuildWeaponSprite weaponSprite = new TeamBuildWeaponSprite(x, y,
                        weapNodeUp, new TouchMessage(MSG_ID.MSG_SCENE_WEAPON_REPAIR__SELECT_WEAPON_UP, null, this, weapNodeUp.id));
        ...
        if (sceneValues.getInt("pageUp") > 1) {
            NormalSprite liftSprite = new NormalSprite(34, 220, "scroolbt", 0, new TouchMessage(MSG_ID.MSG_SCENE_WEAPON_REPAIR__UP_LEFT, null, this));
            addSprite(liftSprite);
        }
        if (sceneValues.getInt("pageUp") < (countUp - 1) / 45 + 1) {
            NormalSprite rightSprite = new NormalSprite(703, 220, "scroolbt", 1, new TouchMessage(MSG_ID.MSG_SCENE_WEAPON_REPAIR__UP_RIGHT, null, this));
            addSprite(rightSprite);
        }

        NormalSprite btnSprite = new NormalSprite(537, 453, "oper_bt4", 1, new TouchMessage(MSG_ID.MSG_SCENE_WEAPON_REPAIR__REPAIR_ALL, null, this));
        addSprite(btnSprite);

        NormalSprite back2Sprite = new NormalSprite(0.0f, 0.0f, "oper_bg3", 0, new TouchMessage(MSG_ID.MSG_SCENE_WEAPON_REPAIR__BACKUP_TOUCH, null, this), 1, 1, 0.0f);
    ...
    public void onHandlMessage(TouchMessage pTouchMessage) {
        ...
        if (msgID == MSG_ID.MSG_SCENE_WEAPON_REPAIR__UP_LEFT) {
            playSound("scrl01");
        ...
        if (msgID == MSG_ID.MSG_SCENE_WEAPON_REPAIR__UP_RIGHT) {
            playSound("scrl01");
            sceneValues.remove("selectedWeaponIDUp");
            sceneValues.setInt("pageUp", sceneValues.getInt("pageUp") + 1);
            WeaponRepairScene weaponRepairScene = new WeaponRepairScene(false);
        ...
        if (msgID == MSG_ID.MSG_SCENE_WEAPON_REPAIR__SELECT_WEAPON_UP) {
        ...
        if (msgID == MSG_ID.MSG_SCENE_WEAPON_REPAIR__REPAIR_ALL) {
            GameDataManager gdm = GameDataManager.getInstance();
//            CustomMenuScene menuSceen = new CustomMenuScene("dialg6");
            ...
            if (priceCount > 0 && priceCount <= gdm.money) {
                ...
                WeaponRepairScene weaponRepairScene = new WeaponRepairScene(false);
                weaponRepairScene.setDialogSecne(new Dialg6YesNoDialogScene(str, MSG_ID.MSG_SCENE_WEAPON_REPAIR__REPAIR_ALL_DLG_YES, MSG_ID.MSG_SCENE_WEAPON_REPAIR__REPAIR_ALL_DLG_NO, this, this));
                SceneManager.render(weaponRepairScene);
            }
        }

        if (msgID == MSG_ID.MSG_SCENE_WEAPON_REPAIR__REPAIR_ALL_DLG_YES) {
            playSound("select01");
            sceneValues.remove("selectedWeaponIDUp");
            GameDataManager gdm = GameDataManager.getInstance();
        ...
        if (msgID == MSG_ID.MSG_SCENE_WEAPON_REPAIR__REPAIR_ALL_DLG_NO) {
            playSound("back01");
            WeaponRepairScene weaponRepairScene = new WeaponRepairScene(false);
            SceneManager.render(weaponRepairScene);
        }

        if (msgID == MSG_ID.MSG_SCENE_WEAPON_REPAIR__BACKUP_TOUCH) {
            sceneValues.remove("selectedWeaponIDUp");
            WeaponRepairScene weaponRepairScene = new WeaponRepairScene(false);
            SceneManager.render(weaponRepairScene);
        }
