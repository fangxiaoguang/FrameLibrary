...
import com.game.gaika.action.SelectBuleSelfDilogYesAction;
 ...
 */
public class BattlefieldScene extends MoveableScene{
    ...
    public BattlefieldScene(boolean isInit) {
        ...
        if (isInit == true) {
            ...
            getLogicCamera().setCente( new Pair<Float, Float>(  cityPoint.getPixelX(), cityPoint.getPixelY()));
    ...
    public void buildScene() {
       // fsmClass = new FSMClass(pCurrentStateID);
        ...
        for (City cityNode : gdm.getCurrentChapter().getGameMap().citys.values()) {
            ...
            if (fsmClass.getCurrentStateID() == STATE_ID_SELECTED_NON) {
                if (cityNode.teamColor == ID.TEAM_COLOR.BLUE && cityNode.isBase == true) {
                    ...
                    if (weapNodes.size() == 0) {
                        msg = new TouchMessage(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_CITY, null, this, cityNode.id);
                    }
                }
            }
        ...
        for (ColorBox box : boxs) {
            TouchMessage msg = null;
            if (fsmClass.getCurrentStateID() == STATE_ID_SELECTED_WEAPON) {
                WeaponSelectFilter filter = new WeaponSelectFilter();
                filter.setMapXY(box.x, box.y);
                filter.setSetOutInBattlefield();
                List<BaseWeapon> weapons = gdm.getWeapons(filter);
                if (weapons.size() == 0) {
                    msg = new TouchMessage(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_DESC, null, this, box.x * 100 + box.y);
                }
            }
        ...
        for (BaseWeapon weapon : weapons) {
            ...
            if (fsmClass.getCurrentStateID() == STATE_ID_SELECTED_NON) {
                ...
                if (city != null && city.teamColor == ID.TEAM_COLOR.BLUE && city.type == ID.CITY_TYPE.AIRPORT && weapon.getPassengers().size() > 0) {
                    msg = new TouchMessage(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_TRANSPORTER, null, this, weapon.id);
                } else if (weapon.teamColor == ID.TEAM_COLOR.BLUE && weapon.moveEnding == false && weapon.canChooseArms() == true) {
                    msg = new TouchMessage(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_CHOOSE_ARMS_WEAPON, null, this, weapon.id);
                } else if (weapon.teamColor == ID.TEAM_COLOR.BLUE && weapon.moveEnding == false) {
                    msg = new TouchMessage(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_WEAPON, null, this, weapon.id);
                } else {
                    msg = new TouchMessage(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_JUST_INFO_WEAPON, null, this, weapon.id);
                }
            }
            if (fsmClass.getCurrentStateID() == STATE_ID_SELECTED_WEAPON) {
                //my self blue weapon
                if (weapon.teamColor == ID.TEAM_COLOR.BLUE && weapon.selected == true) {
                    msg = new TouchMessage(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_SELF, null, this, weapon.id);
                }
                //weapon weapon
                if (gdm.getCurrentChapter().side2.contains(weapon.teamColor) == true
                    ...
                    msg = new TouchMessage(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_ENEMY, null, this, weapon.id);
                }
            ...
            if (fsmClass.getCurrentStateID() == STATE_ID_MOVE_TO_DESC) {
                //my self blue weapon
                if (weapon.teamColor == ID.TEAM_COLOR.BLUE && weapon.selected == true) {
                    msg = new TouchMessage(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_SELF, null, this, weapon.id);
                }
...
//        }
    ...
    public void onHandlMessage(TouchMessage pTouchMessage) {
        ...
        fsmClass.stateTransition(msgID);
...
//            if (msgID == MSG_ID.MSG_SCENE_HUD__BUTTON_BACKUP) {
//                int cityID = pTouchMessage.getParam();
//                BaseAction act = new SelectBuleCityAction(cityID);
//                act.doAction();
//            }

            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_CITY) {
                int cityID = pTouchMessage.getParam();
                BaseAction act = new SelectBuleCityAction(cityID);
                act.doAction();
            }

            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_WEAPON) {
                  playSound("unit01a");
                int weaponID = pTouchMessage.getParam();
                BaseAction act = new SelectBuleWeaponAction(weaponID);
                act.doAction();
            }
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_JUST_INFO_WEAPON) {
                  playSound("unit01a");
                int weaponID = pTouchMessage.getParam();
                BaseAction act = new SelectJustInfoWeaponAction(weaponID);
                act.doAction();
            }
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_CHOOSE_ARMS_WEAPON) {
                  playSound("unit01a");
                int weaponID = pTouchMessage.getParam();
                BaseAction act = new SelectChooseArmsWeaponAction(weaponID);
                act.doAction();
            }
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_CHOOSE_ARMS_DLG_AIR) {
                int weaponID = pTouchMessage.getParam();
                BaseAction act = new SelectChooseArmsDlgAirWeaponAction(weaponID);
                act.doAction();
            }
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_CHOOSE_ARMS_DLG_GROUND) {
                int weaponID = pTouchMessage.getParam();
                BaseAction act = new SelectChooseArmsDlgGroundWeaponAction(weaponID);
                act.doAction();
            }
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_TRANSPORTER) {
                  playSound("unit01a");
                int transporterID = pTouchMessage.getParam();
                BaseAction act = new SelectTransporterOutAction(transporterID);
                act.doAction();
            }
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_PASSENGER) {
                int weaponID = pTouchMessage.getParam();
                BaseAction act = new SelectPassengerAction(weaponID);
                act.doAction();
            }
            if (msgID == MSG_ID.MSG_SCENE_HUD__TURN_FINISH) {
                  playSound("messag01");
                BaseAction act = new TurnFinishAction();
                act.doAction();
            }
            if (msgID == MSG_ID.MSG_SCENE_HUD__TURN_FINISH_DILOG_YES) {
...
//                }
            ...
            if (msgID == MSG_ID.MSG_SCENE_HUD__TURN_FINISH_DILOG_NO) {
                  playSound("back01");
                BaseAction act = new TurnFinishDilogNoAction();
                act.doAction();
            }
            if (msgID == MSG_ID.MSG_SCENE_HUD__BUTTON_SYSTEM) {
                  playSound("select01");
        ...
        if (oldState == STATE_ID_SELECTED_WEAPON) {
            ...
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_SELF) {
                  playSound("messag01");
                BaseAction act = new SelectBuleSelfAction();
                act.doAction();
            }
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_SELF_DLG_YES) {
                  playSound("select01");
                BaseAction act = new SelectBuleSelfDilogYesAction();
                act.doAction();
            }
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_SELF_DLG_NO) {
                  playSound("back01");
                BaseAction act = new SelectBuleSelfDilogNo();
                act.doAction();
            }

            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_DESC) {
                int boxID = pTouchMessage.getParam();
                BaseAction act = new SelectGrrenBoxAction(boxID);
                act.doAction();
            }

            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_ENEMY) {
        ...
        if (oldState == STATE_ID_FIGHT) {
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__FIGHT_TIME_OUT) {
                int weaponID = pTouchMessage.getParam();
        ...
        if (oldState == STATE_ID_LV_UP) {
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__LV_UP_TIME_OUT) {
                int weaponID = pTouchMessage.getParam();
        ...
        if (oldState == STATE_ID_CAPTURING) {
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__CAPTURING_TIME_OUT) {
                int weaponID = pTouchMessage.getParam();
        ...
        if (oldState == STATE_ID_GET_SOME_THING) {
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__GET_SOME_THING_TIME_OUT) {
                int weaponID = pTouchMessage.getParam();
        ...
        if (oldState == STATE_ID_CHECK_WIN) {
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__CHECK_WIN_TIME_OUT) {
                int weaponID = pTouchMessage.getParam();
        ...
        if (oldState == STATE_ID_CRASH) {
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__CRASH_NEXT) {
                BaseAction act = new CrashNextAction();
                act.doAction();
            }
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__CRASH_TIME_OUT) {
        ...
        if (oldState == STATE_ID_AI_TURN_BEGIN) {
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__BLUE_TURN_BEGIN_TIME_OUT) {
                BaseAction act = new BlueTurnBeginTimeOutAction();
                act.doAction();
            }
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__AI_TURN_BEGIN_TIME_OUT) {
                BaseAction act = new AiTurnBeginTimeOutAction();
                act.doAction();
            }
        }
        if (oldState == STATE_ID_AI_REPAIR) {
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__AI_REPAIR_NEXT) {
                BaseAction act = new AiRepairNextAction();
                act.doAction();
            }
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__AI_REPAIR_TIME_OUT) {
        ...
        if (oldState == STATE_ID_AI_BUY) {
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__AI_BUY_NEXT) {
                BaseAction act = new AiBuyNextAction();
                act.doAction();
            }
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__AI_BUY_TIME_OUT) {
                BaseAction act = new AiBuyTimeOutAction();
                act.doAction();
            }
        }
        if (oldState == STATE_ID_AI_SELECT_NON) {
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__AI_SELECT_NON_NEXT) {
                BaseAction act = new AiSelectNonNextAction();
                act.doAction();
            }
        }
        if (oldState == STATE_ID_AI_SELECTED_WEAPON) {
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__AI_SELECT_WEAPON_TIME_OUT) {
                int aiWeaponID = pTouchMessage.getParam();
                BaseAction act = new AiSelectWeaponTimeOutAction(aiWeaponID);
                act.doAction();
            }
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__AI_SELECT_NON_TIME_OUT) {
                int aiWeaponID = pTouchMessage.getParam();
        ...
        if (oldState == STATE_ID_AI_MOVE_TO_DESC) {
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__AI_MOVE_TO_DESC_TIME_OUT) {
                int aiWeaponID = pTouchMessage.getParam();
        ...
        if (oldState == STATE_ID_AI_FIGHT) {
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__AI_FIGHT_TIME_OUT) {
                int aiWeaponID = pTouchMessage.getParam();
        ...
        if (oldState == STATE_ID_BLUE_REPAIR) {
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__BLUE_REPAIR_NEXT) {
                //找到一个可以修理的单位， 显示对话框。
                BaseAction act = new BlueRepairNextAction();
                act.doAction();
            }
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__BLUE_REPAIR_YES) {
                //修理，然后打mark， next
                  playSound("select01");
                int weaponID = pTouchMessage.getParam();
            ...
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__BLUE_REPAIR_NO) {
                //不修理，然后打mark， next
                  playSound("back01");
                BaseAction act = new BlueRepairNoAction();
                act.doAction();
            }
            if (msgID == MSG_ID.MSG_SCENE_BATTLEFIELD__BLUE_REPAIR_TIME_OUT) {
    ...
    public void buildFSM(Enum pCurrentStateID) {
        ...
        stateNon1.addTransition(MSG_ID.MSG_SCENE_HUD__BUTTON_BACKUP, STATE_ID_SELECTED_NON);
        stateNon1.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_CITY, STATE_ID_SELECTED_NON);
        stateNon1.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_WEAPON, STATE_ID_SELECTED_WEAPON);
        stateNon1.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_JUST_INFO_WEAPON, STATE_ID_SELECTED_NON);

        stateNon1.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_CHOOSE_ARMS_WEAPON, STATE_ID_SELECTED_NON);
        stateNon1.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_CHOOSE_ARMS_DLG_AIR, STATE_ID_SELECTED_WEAPON);
        stateNon1.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_CHOOSE_ARMS_DLG_GROUND, STATE_ID_SELECTED_WEAPON);

        stateNon1.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_TRANSPORTER, STATE_ID_SELECTED_NON);
        stateNon1.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_PASSENGER, STATE_ID_SELECTED_WEAPON);

        stateNon1.addTransition(MSG_ID.MSG_SCENE_HUD__TURN_FINISH, STATE_ID_SELECTED_NON);
        stateNon1.addTransition(MSG_ID.MSG_SCENE_HUD__TURN_FINISH_DILOG_YES, STATE_ID_CRASH);
        stateNon1.addTransition(MSG_ID.MSG_SCENE_HUD__TURN_FINISH_DILOG_NO, STATE_ID_SELECTED_NON);

        stateNon1.addTransition(MSG_ID.MSG_SCENE_HUD__BUTTON_SYSTEM, STATE_ID_SELECTED_NON);

        stateNon1.addTransition(MSG_ID.MSG_SCENE_HUD__GAME_OVER_DILOG_YES, STATE_ID_SELECTED_NON);
        stateNon1.addTransition(MSG_ID.MSG_SCENE_HUD__GAME_OVER_DILOG_NO, STATE_ID_SELECTED_NON);
        fsmClass.addState(stateNon1);


        FSMState stateNon2 = new FSMState(STATE_ID_SELECTED_WEAPON);
        stateNon2.addTransition(MSG_ID.MSG_SCENE_HUD__BUTTON_BACKUP, STATE_ID_SELECTED_NON);

        stateNon2.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_SELF, STATE_ID_SELECTED_WEAPON);
        stateNon2.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_SELF_DLG_YES, STATE_ID_LV_UP);
        stateNon2.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_SELF_DLG_NO, STATE_ID_SELECTED_WEAPON);

        stateNon2.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_DESC, STATE_ID_MOVE_TO_DESC);
        stateNon2.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_ENEMY, STATE_ID_FIGHT);
        stateNon2.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_TRANSPORTER, STATE_ID_SELECTED_NON);
        fsmClass.addState(stateNon2);


        FSMState stateNon3 = new FSMState(STATE_ID_MOVE_TO_DESC);
        stateNon3.addTransition(MSG_ID.MSG_SCENE_HUD__BUTTON_BACKUP, STATE_ID_SELECTED_WEAPON);
        stateNon3.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_SELF, STATE_ID_MOVE_TO_DESC);
        stateNon3.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_SELF_DLG_YES, STATE_ID_LV_UP);
        stateNon3.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_SELF_DLG_NO, STATE_ID_MOVE_TO_DESC);
        stateNon3.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__SELECT_ENEMY, STATE_ID_FIGHT);
        fsmClass.addState(stateNon3);


        FSMState stateNon4 = new FSMState(STATE_ID_FIGHT);
        stateNon4.addTransition(MSG_ID.MSG_SCENE_HUD__BUTTON_BACKUP, STATE_ID_FIGHT);
        stateNon4.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__FIGHT_TIME_OUT, STATE_ID_LV_UP);
        fsmClass.addState(stateNon4);

        FSMState stateNon5 = new FSMState(STATE_ID_LV_UP);
        stateNon5.addTransition(MSG_ID.MSG_SCENE_HUD__BUTTON_BACKUP, STATE_ID_LV_UP);
        stateNon5.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__LV_UP_TIME_OUT, STATE_ID_CAPTURING);
        fsmClass.addState(stateNon5);

        FSMState stateNon6 = new FSMState(STATE_ID_CAPTURING);
        stateNon6.addTransition(MSG_ID.MSG_SCENE_HUD__BUTTON_BACKUP, STATE_ID_CAPTURING);
        stateNon6.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__CAPTURING_TIME_OUT, STATE_ID_GET_SOME_THING);
        fsmClass.addState(stateNon6);

        FSMState stateNon7 = new FSMState(STATE_ID_GET_SOME_THING);
        stateNon7.addTransition(MSG_ID.MSG_SCENE_HUD__BUTTON_BACKUP, STATE_ID_GET_SOME_THING);
        stateNon7.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__GET_SOME_THING_TIME_OUT, STATE_ID_CHECK_WIN);
        fsmClass.addState(stateNon7);


        FSMState stateNon8 = new FSMState(STATE_ID_CHECK_WIN);
        stateNon8.addTransition(MSG_ID.MSG_SCENE_HUD__BUTTON_BACKUP, STATE_ID_CHECK_WIN);
        stateNon8.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__CHECK_WIN_TIME_OUT, STATE_ID_SELECTED_NON);
        fsmClass.addState(stateNon8);

        FSMState stateNon9 = new FSMState(STATE_ID_CRASH);
        stateNon9.addTransition(MSG_ID.MSG_SCENE_HUD__BUTTON_BACKUP, STATE_ID_CRASH);
        stateNon9.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__CRASH_NEXT, STATE_ID_CRASH);
        stateNon9.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__CRASH_TIME_OUT, STATE_ID_AI_TURN_BEGIN);
        fsmClass.addState(stateNon9);

        //----------------AI loop begin---------

        FSMState stateNon10 = new FSMState(STATE_ID_AI_TURN_BEGIN);
        stateNon10.addTransition(MSG_ID.MSG_SCENE_HUD__BUTTON_BACKUP, STATE_ID_AI_TURN_BEGIN);
        stateNon10.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__BLUE_TURN_BEGIN_TIME_OUT, STATE_ID_BLUE_REPAIR);
        stateNon10.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__AI_TURN_BEGIN_TIME_OUT, STATE_ID_AI_REPAIR);
        fsmClass.addState(stateNon10);

        FSMState stateNon11 = new FSMState(STATE_ID_AI_REPAIR);
        stateNon11.addTransition(MSG_ID.MSG_SCENE_HUD__BUTTON_BACKUP, STATE_ID_AI_REPAIR);
        stateNon11.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__AI_REPAIR_NEXT, STATE_ID_AI_REPAIR);
        stateNon11.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__AI_REPAIR_TIME_OUT, STATE_ID_AI_BUY);
        fsmClass.addState(stateNon11);

        FSMState stateNon11_2 = new FSMState(STATE_ID_AI_BUY);
        stateNon11_2.addTransition(MSG_ID.MSG_SCENE_HUD__BUTTON_BACKUP, STATE_ID_AI_BUY);
        stateNon11_2.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__AI_BUY_NEXT, STATE_ID_AI_BUY);
        stateNon11_2.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__AI_BUY_TIME_OUT, STATE_ID_AI_SELECT_NON);
        fsmClass.addState(stateNon11_2);


        FSMState stateNon12 = new FSMState(STATE_ID_AI_SELECT_NON);
        stateNon12.addTransition(MSG_ID.MSG_SCENE_HUD__BUTTON_BACKUP, STATE_ID_AI_SELECT_NON);
        stateNon12.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__AI_SELECT_NON_NEXT, STATE_ID_AI_SELECTED_WEAPON);
//        stateNon12.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__AI_SELECT_NON_TIME_OUT, STATE_ID_AI_CRASH);
        fsmClass.addState(stateNon12);

        FSMState stateNon13 = new FSMState(STATE_ID_AI_SELECTED_WEAPON);
        stateNon13.addTransition(MSG_ID.MSG_SCENE_HUD__BUTTON_BACKUP, STATE_ID_AI_SELECTED_WEAPON);
        stateNon13.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__AI_SELECT_WEAPON_TIME_OUT, STATE_ID_AI_MOVE_TO_DESC);
        stateNon13.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__AI_SELECT_NON_TIME_OUT, STATE_ID_AI_CRASH);
        fsmClass.addState(stateNon13);

        FSMState stateNon14 = new FSMState(STATE_ID_AI_MOVE_TO_DESC);
        stateNon14.addTransition(MSG_ID.MSG_SCENE_HUD__BUTTON_BACKUP, STATE_ID_AI_MOVE_TO_DESC);
        stateNon14.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__AI_MOVE_TO_DESC_TIME_OUT, STATE_ID_AI_FIGHT);
        fsmClass.addState(stateNon14);


        FSMState stateNon15 = new FSMState(STATE_ID_AI_FIGHT);
        stateNon15.addTransition(MSG_ID.MSG_SCENE_HUD__BUTTON_BACKUP, STATE_ID_AI_FIGHT);
        stateNon15.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__AI_FIGHT_TIME_OUT, STATE_ID_AI_LV_UP);
        fsmClass.addState(stateNon15);

        FSMState stateNon16 = new FSMState(STATE_ID_AI_LV_UP);
        stateNon16.addTransition(MSG_ID.MSG_SCENE_HUD__BUTTON_BACKUP, STATE_ID_AI_LV_UP);
        stateNon16.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__LV_UP_TIME_OUT, STATE_ID_AI_CAPLTURING);
        fsmClass.addState(stateNon16);

        FSMState stateNon17 = new FSMState(STATE_ID_AI_CAPLTURING);
        stateNon17.addTransition(MSG_ID.MSG_SCENE_HUD__BUTTON_BACKUP, STATE_ID_AI_CAPLTURING);
        stateNon17.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__CAPTURING_TIME_OUT, STATE_ID_AI_CHECK_WIN);
        fsmClass.addState(stateNon17);

        FSMState stateNon18 = new FSMState(STATE_ID_AI_CHECK_WIN);
        stateNon18.addTransition(MSG_ID.MSG_SCENE_HUD__BUTTON_BACKUP, STATE_ID_AI_CHECK_WIN);
        stateNon18.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__CHECK_WIN_TIME_OUT, STATE_ID_AI_SELECT_NON);
        fsmClass.addState(stateNon18);

        FSMState stateNon19 = new FSMState(STATE_ID_AI_CRASH);
        stateNon19.addTransition(MSG_ID.MSG_SCENE_HUD__BUTTON_BACKUP, STATE_ID_AI_CRASH);
        stateNon19.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__CRASH_NEXT, STATE_ID_AI_CRASH);
        stateNon19.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__CRASH_TIME_OUT, STATE_ID_AI_TURN_BEGIN);
        fsmClass.addState(stateNon19);
        //-----------------------------
        FSMState stateNon20 = new FSMState(STATE_ID_BLUE_REPAIR);
        stateNon20.addTransition(MSG_ID.MSG_SCENE_HUD__BUTTON_BACKUP, STATE_ID_BLUE_REPAIR);
        stateNon20.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__BLUE_REPAIR_NEXT, STATE_ID_BLUE_REPAIR);
        stateNon20.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__BLUE_REPAIR_YES, STATE_ID_BLUE_REPAIR);
        stateNon20.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__BLUE_REPAIR_NO, STATE_ID_BLUE_REPAIR);
        stateNon20.addTransition(MSG_ID.MSG_SCENE_BATTLEFIELD__BLUE_REPAIR_TIME_OUT, STATE_ID_SELECTED_NON);
        fsmClass.addState(stateNon20);
