...
public class AiMoveToDescTimeOutAction implements BaseAction {
    ...
    public void doAction() {
        ...
        if (aiWeapon.getStrategic().getEnemy() == null || aiWeapon.canAttack(aiWeapon.getStrategic().getEnemy()) == false) {
            battlefieldScene.addSprite(new DelaySprite(GameSetup.DELAY_SHORT_S, new TouchMessage(MSG_ID.MSG_SCENE_BATTLEFIELD__AI_FIGHT_TIME_OUT, null, battlefieldScene, aiWeaponID)));
        } else {
...
//            enemy.doFight(aiWeapon);
            ...
            battlefieldScene.addSprite(new DelaySprite(GameSetup.DELAY_AI_LOOP_S, new TouchMessage(MSG_ID.MSG_SCENE_BATTLEFIELD__AI_FIGHT_TIME_OUT, null, battlefieldScene, aiWeaponID)));
        }
